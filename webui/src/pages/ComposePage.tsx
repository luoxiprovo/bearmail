...
import { composeDraftBody, destroyEmail, forwardedMessage, forwardSubject, getEmail, identitySignatureText, replyQuote, replySubject, saveDraft, sendDraft } from "../jmap/mail";
...
export function ComposePage({ draftId: routeDraftId }: { draftId?: string }) {
  ...
  useEffect(() => {
    ...
    setInput((current) => current.body === unsignedBody ? { ...current, body: seededBody } : current);
  }, [routeDraftId, seededBody, unsignedBody]);
  ...
  useEffect(() => {
    if (!client || !identity || !draftMailbox || (!input.to && !input.subject && !attachments.length && input.body === seededBody)) return;
  ...
  }, [attachments, client, draftMailbox, identity, input, notify, seededBody]); // draftId intentionally excluded to avoid a save loop
  ...
  return (
    <form className="compose-page" onSubmit={submit}>
      ...
      <div className="compose-sheet">
        ...
        <textarea ref={bodyField} className="compose-body" aria-label="Message body" placeholder="Start writing…" value={input.body} onChange={(event) => update("body", event.target.value)} />
