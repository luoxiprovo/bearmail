...
5. Scheduling: `get_availability`, then `create_event` with `attendees`. For an irregular series (sports schedule), pass `occurrences` on the same call — one UID, one invitation. Those dates become iCalendar `RDATE` on a single VEVENT (the shape Gmail accepts). Do not invent a weekly `RRULE` plus exception VEVENTs for mixed kickoff times. Use `recurrence` only for a true regular pattern. Success means JMAP accepted it, not that the guest opened it.
